 ...
 * @param {SourceCode} sourceCode
 * @param {boolean} enhancedSorting
 */
function isAlphabetical(prevNode, currNode, sourceCode, enhancedSorting) {
  const prevName = getAttributeName(prevNode, sourceCode)
  const currName = getAttributeName(currNode, sourceCode)
  if (enhancedSorting) {
    const prevHasValue = Boolean(prevNode.value)
    const currHasValue = Boolean(currNode.value)
    if (prevHasValue !== currHasValue) {
      return prevHasValue
    }

    const isPrevClass = prevName === 'class'
    const isCurrClass = currName === 'class'
    if (isPrevClass !== isCurrClass) {
      return isPrevClass
    }

    const prevDirective = prevNode.directive ? prevNode.key.name.name : ''
    const currDirective = currNode.directive ? currNode.key.name.name : ''
    if (prevDirective !== currDirective) {
      return prevDirective < currDirective
    }
  }
...
function create(context) {
  ...
  if (context.options[0] && context.options[0].order) {
    ...
    for (const item of attributeOrder.flat()) {
      if (item === ATTRS.OTHER_ATTR) {
        for (const attribute of attributeOrder.flat()) {
          if (OTHER_ATTRS.includes(attribute)) {
            throw new Error(
    ...
    for (const [index, item] of attributeOrder.entries()) {
      if (item === ATTRS.OTHER_ATTR) {
        attributeOrder[index] = OTHER_ATTRS
      } else if (Array.isArray(item) && item.includes(ATTRS.OTHER_ATTR)) {
        const attributes = item.filter((i) => i !== ATTRS.OTHER_ATTR)
        attributes.push(...OTHER_ATTRS)
        attributeOrder[index] = attributes
      }
    }
  }

  const alphabetical = !!context.options[0]?.alphabetical
  const enhancedAlphabeticalSorting = !!(
    context.options[0]?.enhancedAlphabeticalSorting ?? true
  )
  ...
  return utils.defineTemplateBodyVisitor(context, {
    VStartTag(node) {
      const attributeAndPositions = getAttributeAndPositionList(
        node,
        attributePosition
      )
      ...
      for (let index = 1; index < attributeAndPositions.length; index++) {
        ...
        if (valid && alphabetical && previousPosition === position) {
          valid = isAlphabetical(
            previousNode,
            attr,
            sourceCode,
            enhancedAlphabeticalSorting
          )
        }
        if (valid) {
          previousNode = attr
    ...
    schema: [
      {
        ...
        properties: {
          ...
          enhancedAlphabeticalSorting: { type: 'boolean' }
        },
