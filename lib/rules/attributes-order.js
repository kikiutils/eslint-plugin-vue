...
function create(context) {
  ...
  const alphabeticalEnhanced = Boolean(
    context.options[0] && context.options[0].alphabeticalEnhanced
  )
      ...
      for (let index = 1; index < attributeAndPositions.length; index++) {
        ...
        if (isValid && previousPosition === position) {
          ...
          if (shouldSortAlphabetical && !isSortedByLength) {
            isValid = isAlphabetical(
              previousNode,
              attr,
              sourceCode,
              alphabeticalEnhanced
            )
          }
        }
    ...
    schema: [
      {
        ...
        properties: {
          ...
          alphabeticalEnhanced: { type: 'boolean' },
